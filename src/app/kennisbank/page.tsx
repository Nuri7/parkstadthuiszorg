  ...
  return (
    <div className="pt-32 pb-20 bg-warm-gradient min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Uitgelichte gidsen (met eigen pagina) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {kennisbankArtikelen.map((artikel, index) => (
            <AnimatedSection key={artikel.slug} delay={index * 0.08}>
              <Link href={`/kennisbank/${artikel.slug}`} className="block h-full">
                <article className="bg-white dark:bg-[#243029] rounded-2xl overflow-hidden shadow-lg border border-[#ede7db] dark:border-[#086370] h-full flex flex-col group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  <div className="p-2">
                    <div className="bg-[#e8f0e9] dark:bg-[#1e2e25] rounded-xl p-6 flex items-center justify-center h-36">
                      <BookOpen className="w-12 h-12 text-[#5b7f63] dark:text-[#7c9a82] opacity-60" />
                    </div>
                  </div>
                  <div className="p-6 flex-grow flex flex-col">
                    <span className="inline-block text-xs font-semibold uppercase tracking-wider text-[#4A9C6E] mb-2">{artikel.category}</span>
                    ...
                    <div className="flex items-center justify-between text-xs text-[#8ab0b6] mt-auto pt-4 border-t border-[#ede7db] dark:border-[#086370]">
                      <div className="flex items-center gap-3">
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{artikel.readTime}</span>
                        <span className="flex items-center gap-1"><User className="w-3 h-3" />{artikel.date}</span>
                      </div>
                      <span className="text-[#5b7f63] font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
                        Lees het artikel <ArrowRight className="w-3 h-3" />
                      </span>
                    </div>
                  </div>
                </article>
        ...
        <AnimatedSection className="text-center max-w-2xl mx-auto mb-10">
          <h2 className="text-2xl font-heading text-[#064a54] dark:text-[#fefdfc] mb-2">
            Binnenkort in de kennisbank
          </h2>
          <p className="text-[#4f6b6f] dark:text-[#5cb0bd] text-sm">
            Aan deze onderwerpen wordt gewerkt. Heeft u er nu al een vraag over? Neem
            gerust contact op.
          </p>
        </AnimatedSection>
              ...
              <article className="bg-white dark:bg-[#243029] rounded-2xl overflow-hidden shadow-lg border border-[#ede7db] dark:border-[#086370] h-full flex flex-col group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="p-6 flex-grow flex flex-col">
                  ...
                  <div className="flex items-center justify-between text-xs text-[#8ab0b6] mt-auto pt-4 border-t border-[#ede7db] dark:border-[#086370]">
                    ...
                    <span className="text-[#8ab0b6] font-medium">
                      Binnenkort
                    </span>
                  </div>
                </div>

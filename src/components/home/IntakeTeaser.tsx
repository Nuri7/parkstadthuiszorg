    ...
    <section className="section-padding bg-white dark:bg-[#1e2e25]">
      <div className="max-w-7xl mx-auto">
        <AnimatedSection className="text-center max-w-2xl mx-auto mb-16">
          <h2 className="text-[#C67D5B] font-semibold tracking-wider text-sm uppercase mb-3">Hoe Werkt Het?</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          <div className="hidden lg:block absolute top-[2.5rem] left-[12%] right-[12%] h-[2px] bg-gradient-to-r from-[#dce8de] via-[#7c9a82] to-[#dce8de] dark:from-[#344a3c] dark:via-[#5b7f63] dark:to-[#344a3c] z-0" />

          {steps.map((step, index) => (
            <AnimatedSection key={index} delay={index * 0.15} className="relative z-10 text-center">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white dark:bg-[#243029] shadow-lg border-2 border-[#f0f5f1] dark:border-[#344a3c] mx-auto flex items-center justify-center mb-6 relative group transition-transform hover:-translate-y-1">
                <step.icon className="w-8 h-8 text-[#5b7f63] dark:text-[#7c9a82]" />
                
                {/* Step Number Badge */}
                <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-[#C67D5B] text-white flex items-center justify-center font-bold text-sm border-4 border-white dark:border-[#1e2e25]">
                  {index + 1}
                </div>
              </div>
              <h4 className="text-xl font-bold text-[#2d3b2d] dark:text-[#fdfbf7] mb-3">
                {step.title.substring(3)} {/* Remove "1. " from title here as we show it in the badge */}
              </h4>
              <p className="text-[#6B7B6B] dark:text-[#94ba9a]">
                {step.text}
              </p>
            </AnimatedSection>
          ))}
        ...
        <AnimatedSection delay={0.6} className="mt-16 text-center">
          <div className="bg-[#f0f5f1] dark:bg-[#243029] p-8 md:p-10 rounded-3xl inline-block max-w-3xl border border-[#dce8de] dark:border-[#344a3c]">
            <h4 className="text-2xl font-heading text-[#2d3b2d] dark:text-[#fdfbf7] mb-4">Direct zorg nodig?</h4>
            <p className="text-[#6B7B6B] dark:text-[#94ba9a] mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button asChild variant="primary" size="lg">
               ...
               <Button asChild variant="outline" size="lg" className="bg-white dark:bg-[#1a2420]">
